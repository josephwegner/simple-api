...
Controller.prototype.checkRoute = function(urlParts) {
	...
	for(action in this.routes) {
		...
		for(i=0, max=route.length; i<max; i++) {
			...
			if(cont) {
				this.private.debug("Route did not match "+curAction, 1);
				break;
			}
		...
		this.private.debug("Found matching route "+ curAction, 1);
		break;
	...
	if(selectedRoute === false) {
		...
		return function(req, res) {
			if(typeof(this.actions[selectedRoute]) === "function") {
				this.actions[selectedRoute](req, res, params);
			} else {
				this.api.fiveHundred(res);
				this.private.debug("We found a matching route, but there is no action for that route!", 3);
