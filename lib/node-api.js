...
var API = function(options) {
	...
	this.private.debug("node-api listening on http://"+this.options.host+":"+this.options.port, 4);
}
	...
	isAPICall: function(req) {
		...
		switch(typeof(this.options.prefix)) {
			...
			case "object":
				...
				if(importantParts === this.options.prefix) {
					return parts.slice(this.options.prefix.length + 1).join("");
				} else {
					return false;
				}
		...
		if((url = this.private.isAPICall.call(this, req)) !== false) {
			...
			**/
			...
			if(typeof(this.controllers[controllerName]) !== "undefined") {
				...
				var route;
				if(typeof(route = controller.checkRoute(parts)) === "function") {
					this.private.debug("Found matching route for API request", 2);
					route(req, res);
				} else {
					this.private.debug("Route does not exist for API request", 2);
					this.fourOhFour(res);
				}

			} else {
				this.private.debug("Controller does not exist for API request", 2);
				this.fourOhFour(res);
			}
...
API.prototype.Controller = function(name, configuration) {
	...
	this.debu("Created controller: " + name, 3);
